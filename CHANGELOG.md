...
## [Unreleased]
### Fixed
- **Double-counting and non-reversible study hours in task toggling**: `toggleTaskCompletion` in `studyPlanController.js` unconditionally added `studyTimeMinutes` to `user.studyHours` whenever `completed: true` was sent, without checking the task's prior completion state. This caused double-counting (repeatedly completing the same task added hours each time) and non-reversible hours (unmarking a task never subtracted time). Fixed by capturing the task's previous `completed` state before the update, then applying state-transition logic: `false→true` adds hours, `true→false` subtracts hours, unchanged state skips mutation. Study hours are floored at 0 to prevent negative values. (#180)
