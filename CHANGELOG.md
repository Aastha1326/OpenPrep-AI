...
## [Unreleased]
### Fixed
- **Infinite incrementing of flashcardsMastered in Progress counter**: Reviewing a flashcard with quality >= 4 always incremented `progress.flashcardsMastered` without checking if the card was already mastered. Added `isMastered` boolean to the Flashcard model. Only increment `flashcardsMastered` when a card transitions from not mastered -> mastered (quality >= 4). Decrement when a card transitions from mastered -> not mastered (quality < 4). Unchanged state skips mutation. Uses `Math.max(0, ...)` when decrementing to prevent negative counts. (#181)
