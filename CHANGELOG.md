...
## [Unreleased]
### Fixed
- **Refresh Token Rotation — old tokens not removed from database after rotation**: When a refresh token was rotated, the old hashed token remained in the user's `refreshTokens` array in the database, allowing replayed tokens to be accepted until they expired. The root cause was that `generateRefreshToken` received only a `userId` string and fetched its own user instance, so it saved the filtered token array to a stale in-memory copy instead of the active Mongoose document. Fixed by PR #309 (passing the `user` object instead of `userId`, ensuring `user.save()` persists the filtered array). Added a regression test that proves: (1) old refresh token is rejected after rotation, (2) intermediate token is also invalidated after second rotation, (3) newest token still works. (#179)
- **Unchecked LLM response properties causing 500 crashes in PYQ analysis**: Gemini API responses can omit nested properties (e.g., `topicName`) inside array items (`importantTopics`), causing `TypeError` on `.trim()` in `pyqController.js`. Enhanced `validateResponse` in `geminiService.js` to recursively validate item shapes (`topicName`, `importance`, `frequency`, `chapterName`, `weightage`, `questionText`, `years`, `correctAnswer`, `options`, `explanation`, `subject`, `suggestion`, `priority`) across all AI response schemas (PYQ analysis, quiz, performance, study plan, flashcard). Also added defensive `continue` guard in `pyqController.js` to skip malformed items. (#184)
