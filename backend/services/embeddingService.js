...
async function generateVector(text) {
  ...
  try {
    ...
    logger.error('[EmbeddingService] Gemini embedding call failed:', err.message);
    throw err;
  }
}
...
async function enqueueEmbeddingUpdate(modelInstance, text) {
  try {
    ...
    logger.info(`[EmbeddingService] Enqueued embedding job for ${modelInstance.constructor.name} ID: ${modelInstance.id}`);
  } catch (err) {
    logger.warn('[EmbeddingService] Failed to enqueue embedding job, executing synchronously:', err.message);
    // Synch fallback if task queue is offline
    try {
      const vector = await generateVector(text);
      modelInstance.embedding = vector;
      await modelInstance.save({ hooks: false });
    } catch (fallbackErr) {
      logger.error('[EmbeddingService] Synch fallback failed:', fallbackErr.message);
    }
  }
}
...
function registerWorkerHandler(models) {
  ...
  queueService.registerHandler('generate_embeddings', async (payload) => {
    ...
    if (!record) {
      logger.warn(`[EmbeddingWorker] Record not found for async embedding: ${modelName} ID: ${recordId}`);
      return;
    }
    ...
    logger.info(`[EmbeddingWorker] Successfully updated embedding vector for ${modelName} ID: ${recordId}`);
  });
}
